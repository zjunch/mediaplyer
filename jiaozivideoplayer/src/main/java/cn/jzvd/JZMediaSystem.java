...
public class JZMediaSystem extends JZMediaInterface implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnBufferingUpdateListener, MediaPlayer.OnSeekCompleteListener, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener, MediaPlayer.OnVideoSizeChangedListener {
    ...
    private boolean isCompletion=false;  //是否完成，全屏时播放完成，返回不需要再播放
    ...
    public long getCurrentPosition() {
        if (mediaPlayer != null) {
            Log.e("zjun","CurrentPosition():"+mediaPlayer.getCurrentPosition());
            return mediaPlayer.getCurrentPosition();
        } else {
            Log.e("zjun","CurrentPosition():null");
            return 0;
        }
    }
    ...
    public void onPrepared(MediaPlayer mediaPlayer) {
        Log.e("Zjun","onPrepared:");
        if(isCompletion){  //如果已经完成了，准备好不需要播放了
            return;
        }
    ...
    public void onBufferingUpdate(MediaPlayer mediaPlayer, final int percent) {
        Log.e("Zjun","onBufferingUpdate:"+percent);
        handler.post(() -> jzvd.setBufferProgress(percent));
    }

    @Override
    public void onSeekComplete(MediaPlayer mediaPlayer) {
        Log.e("Zjun","onSeekComplete:");
